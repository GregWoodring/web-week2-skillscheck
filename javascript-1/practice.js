...
function bigOrSmall(arr) {
	let answers = arr.map((item) => item > 100 ? 'big' : 'small');
	return answers;
}
...
let doubled = myNumbers.map((item) => item*2);
...
// Code Here
let filtered = myNumbers.filter((item) => item > 100)
//////////////////PROBLEM 16////////////////////

// Next up, reduce to get the total of the array. call the new array 'total'

// Code Here
let total = myNumbers.reduce((acc, item) => acc + item);
...
let enemies = people.filter((item) => !item.friend);
...
let totallyAwesome = people.reduce((acc, item) => acc + item.awesomeLevel, 0);
